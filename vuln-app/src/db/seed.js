...
function seed() {
  ...
  // VULNERABILITY: Passwords stored as plain text (no hashing)
  db.prepare(`INSERT INTO users (username, email, password, role, bio) VALUES (?, ?, ?, ?, ?)`).run(
    'alice', 'alice@vulnshop.lab', 'password123', 'user', 'Hi, I am Alice! I love shopping online.'
  );
  db.prepare(`INSERT INTO users (username, email, password, role, bio) VALUES (?, ?, ?, ?, ?)`).run(
    'bob', 'bob@vulnshop.lab', 'bobspassword', 'user', 'Bob here. Bargain hunter!'
  );
  db.prepare(`INSERT INTO users (username, email, password, role, bio) VALUES (?, ?, ?, ?, ?)`).run(
    'admin', 'admin@vulnshop.lab', 'admin1234', 'admin', 'Site administrator.'
  );
  ...
  // Seed orders for alice (user id 1)
  ...
  // Seed reviews
  db.prepare(`INSERT INTO reviews (product_id, user_id, username, rating, comment) VALUES (?, ?, ?, ?, ?)`).run(
    1, 2, 'bob', 5, 'Amazing headphones! Sound quality is superb.'
  );
  db.prepare(`INSERT INTO reviews (product_id, user_id, username, rating, comment) VALUES (?, ?, ?, ?, ?)`).run(
    1, 1, 'alice', 4, 'Great product, comfortable to wear for long sessions.'
  );
  db.prepare(`INSERT INTO reviews (product_id, user_id, username, rating, comment) VALUES (?, ?, ?, ?, ?)`).run(
    2, 1, 'alice', 5, 'Super lightweight, perfect for my morning runs!'
  );

  // Seed support ticket
  db.prepare(`INSERT INTO support_tickets (user_id, name, email, subject, message) VALUES (?, ?, ?, ?, ?)`).run(
    1, 'Alice', 'alice@vulnshop.lab', 'Order question', 'When will my order arrive?'
  );
